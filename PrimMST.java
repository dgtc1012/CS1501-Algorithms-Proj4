...
public class PrimMST {
    ...
    private void scan(Graph G, int v) {
        marked[v] = true;
        for (Edge e : G.adjMST(v)) {
            int w = e.other(v+1);
            if (marked[w-1]) continue;         // v-w is obsolete edge
            if (e.weight() < distTo[w-1]) {
                distTo[w-1] = e.weight();
                edgeTo[w-1] = e;
                if (pq.contains(w-1)){
                   pq.decreaseKey(w-1, distTo[w-1]);
                }
                else{    
                    pq.insert(w-1, distTo[w-1]);
                }
            }
        }
    }
